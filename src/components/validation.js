export const enableValidation = (config) => {
  const formList = Array.from(
    document.querySelectorAll(config.popupFormElement)
  );
...
const showInputError = (inputElement, errorMessage, config) => {
  const errorElement = inputElement
    .closest(config.popupFormElement)
    .querySelector(`#${inputElement.id} + ${config.inputError}`);
...
export function clearValidationErrors(formElement, config) {
  ...
  toggleButtonState(
    inputList,
    formElement.querySelector(config.button),
    config
  );
}
